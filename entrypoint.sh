...
PYTHONPATH=$PYTHONPATH:/home/sonic
export PYTHONPATH
...
echo "Setting git safe.directory to /home/sonic/lint ..."
git config --global --add safe.directory /home/sonic/lint
...
if [ "${UPGRADE_LINTERS_VERSION}" == "true" ]; then
  ...
  cd /home/sonic/lint || exit 1
  chmod +x build.sh
  GITHUB_TOKEN="${GITHUB_TOKEN}" bash build.sh --doc --dependents --stats
  exit $?
fi

# creation of mega-linter.yml from env to make it accessible from CI (by sonic)

# Check if the environment variable is set
if [ -z "$MEGALINTER_CONF" ]; then
  echo "Environment variable MEGALINTER_CONF is not set."
  exit 1
fi

# Create the .mega-linter.yml file with the content from the environment variable
#echo "$MEGALINTER_CONF" > /home/sonic/lint/.mega-linter.yml
# Create the .mega-linter.yml file with the content from the environment variable
printf "%b" "$MEGALINTER_CONF" > /home/sonic/lint/.mega-linter.yml

echo ".mega-linter.yml file created successfully:."
cat /home/sonic/lint/.mega-linter.yml
